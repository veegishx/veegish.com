...
export async function getRecentCommits(): Promise<Commit[]> {
  try {
    ...
    for (let i = 0; i < Math.min(repos.length, 5); i++) {
      if (allCommits.length >= 10) break;

      const repo = repos[i];
      try {
        const commitsResponse = await fetch(
          `${GITHUB_API}/repos/${repo.full_name}/commits?per_page=5`,
          { headers: getHeaders() }
        );
    ...
    console.log("Fetched commits:", allCommits.length);
  ...
  try {
    ...
    return {
      ...
      contributions: data.contributions.map(
        (c: { date: string; level: number; count: number }) => ({
          date: c.date,
          level: c.level,
...
export function getEventDescription(event: GitHubEvent): string {
  ...
  switch (event.type) {
    ...
    case "IssuesEvent": {
      const issueAction =
        event.payload.action === "opened" ? "opened" : event.payload.action;
